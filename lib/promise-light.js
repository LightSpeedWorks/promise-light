...
this.PromiseLight = function () {
  ...
  var error = (console.error || console.log).bind(console);
  ...
  function Promise(setup) {
    ...
    function checkUnhandledRejection() {
      if (state === REJECTED && !handled) {
        error(COLOR_ERROR + 'Unhandled rejection ' +
...
this.Promise = this.Promise || Function('return this')().Promise || this.PromiseLight;
